...
class anasayfa_sql extends load
{
    ...
    function bankaekle($ad)
    {
        $tablo = "bankalar";
        $data["ad"] = $ad;
        $diger_id = $this->db->get_var("SELECT id FROM bankalar WHERE ad = 'DİĞER' ");
        $id = $this->insert("bankalar", $data);
        $this->update($tablo, $data, "id = $diger_id");
        $data["ad"] = "DİĞER";
        $this->update($tablo, $data, "id = $id");
        return $this->db->get_results("select * from bankalar order by id asc ");
    ...
    function b_k_a_d($id)
    {
        return $this->db->get_var("select acilis_doviz from banka_kasa where id = $id ");
    }
    function b_k_bakiye_update($miktar,$id){
