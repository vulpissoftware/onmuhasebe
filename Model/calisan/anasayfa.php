...
class anasayfa_sql extends load
{
    ...
    function calisanlar($sira = 1, $gosterim_adet = SAYFALAMA_ADET)
    {
        $this->data->toplam = $this->db->get_var("select count(id) from calisan ");
        $this->data->sayfa = ceil($this->data->toplam / $gosterim_adet);
        $ilk = ($sira - 1) * $gosterim_adet;
        $this->data->veri = $this->db->get_results("select * from calisan  LIMIT $ilk , $gosterim_adet");
        return $this->data;
    }

    function calisansil($mid)
    {
        $this->yetkilisi($mid);
        $this->db->query("DELETE FROM calisan WHERE id = $mid");
        return $mid;
    ...
    function calisan($id)
    {
        return $this->db->get_row("select * from calisan where id = $id");
    }

    function calisanupdate($id, $veri)
    {
        $this->update("calisan", $veri, "id = $id");
    }

    function maasupdate($id, $veri)
    {
        $this->update("calisan_maas", $veri, "id = $id");
    }

    function yetkilisi($mid)
    {
        $this->db->query("DELETE FROM tedarikci_yetkili WHERE musteri_id = $mid");
    }

    function yetkililer($mid)
    {
        return $this->db->get_results("select * from tedarikci_yetkili  where musteri_id = $mid  ");
    }
    ...
    /* TAYLAN */
    function calisankategori()
    {
        return $this->db->get_results("select * from calisan_kategori   ");

    }
    function tl_kasalar()
    {
        return $this->db->get_results("select * from banka_kasa where arsiv=0 AND acilis_doviz='TL'");
    }
    function kasa_cek($id)
    {
        return $this->db->get_row("select * from banka_kasa where id=$id");
    }
    function maas_cek($id)
    {
        return $this->db->get_row("select * from calisan_maas where id=$id");
    }
    function ay_maaslar($calisan, $id){
        return $this->db->get_results("select * from calisan_maas WHERE calisan=$calisan AND odeme_durumu=0 AND id!=$id");
    }
    function ensonay($calisan){
        return $this->db->get_row("select * from calisan_maas WHERE calisan=$calisan AND odeme_durumu=0 ORDER BY tarih ASC LIMIT 1");
    }
    function maaslar($calisan){
        return $this->db->get_results("select * from calisan_maas WHERE calisan=$calisan AND odeme_durumu=0 ORDER BY odenecek_tarih ASC");
    }
    /* TAYLAN */
