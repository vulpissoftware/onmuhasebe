...
class anasayfa_sql extends load
{
    ...
    function tedarikciler($sira = 1, $gosterim_adet = SAYFALAMA_ADET)
    {
        $this->data->toplam = $this->db->get_var("select count(id) from tedarikciler ");
        $this->data->sayfa = ceil($this->data->toplam / $gosterim_adet);
        $ilk = ($sira - 1) * $gosterim_adet;
        $this->data->veri = $this->db->get_results("select * from tedarikciler  LIMIT $ilk , $gosterim_adet");
        return $this->data;
    }

    function tedarikcisil($mid)
    {
        $this->yetkilisi($mid);
        $this->db->query("DELETE FROM tedarikciler WHERE id = $mid");
        return $mid;

    }

    function tedarikci($id)
    {
        return $this->db->get_row("select * from tedarikciler where id = $id");
    }

    function tedarikciupdate($id, $veri)
    {
        $this->update("tedarikciler", $veri, "id = $id");
    }
